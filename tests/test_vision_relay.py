...
async def test_describe_camera_frame_sends_question_and_image(monkeypatch: Any) -> None:
    ...
    monkeypatch.setattr(config, "DASHSCOPE_CHAT_BASE", "https://dashscope.aliyuncs.com/compatible-mode/v1")
...
@pytest.mark.asyncio
async def test_describe_camera_frame_uses_configured_chat_base(monkeypatch: Any) -> None:
    """The vision relay posts to DASHSCOPE_CHAT_BASE when it is set."""
    monkeypatch.setattr(config, "DASHSCOPE_API_KEY", "my-key")
    monkeypatch.setattr(config, "DASHSCOPE_CHAT_BASE", "https://token-plan.example/v1/")
    monkeypatch.setattr(vision_relay.httpx, "AsyncClient", _FakeAsyncClient)
