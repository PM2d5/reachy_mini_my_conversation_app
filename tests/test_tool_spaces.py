...
from my_conversation_app.mcp_client import RemoteToolSpec, RemoteMcpToolClient
...
def test_tool_spaces_add_list_remove_round_trip(
    ...
    monkeypatch.setattr("my_conversation_app.tool_spaces.resolve_tool_space_sync", resolve_tool_space)
...
def test_resolve_tool_space_sends_auth_only_to_private_spaces(
    ...
    monkeypatch.setattr("my_conversation_app.tool_spaces.get_token", lambda: login_token)
...
def _mock_add(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    monkeypatch.chdir(tmp_path)
    monkeypatch.setattr("my_conversation_app.profile_store.DEFAULT_PROFILES_DIRECTORY", tmp_path)
    monkeypatch.setattr(
        "my_conversation_app.tool_spaces.HfApi.space_info",
        lambda self, slug, **kwargs: _mock_public_space_info(slug),
    )
...
def test_tool_space_install_profile_failure_leaves_manifest_unchanged(
    ...
    monkeypatch.setattr(config_mod.config, "PROFILES_DIRECTORY", tmp_path)
    monkeypatch.setattr(
        "my_conversation_app.tool_spaces.enable_profile_tools",
...
def test_tool_space_install_manifest_failure_rolls_back_profile(
    ...
    monkeypatch.setattr(config_mod.config, "PROFILES_DIRECTORY", tmp_path)
    monkeypatch.setattr(
        "my_conversation_app.tool_spaces.write_installed_tool_spaces",
