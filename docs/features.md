...
| `REACHY_MINI_SPEAKER_MATCH_THRESHOLD` | `0.60` | 声纹余弦匹配阈值（0–1，调高更严格）；低于 1.5 s 的语音段不判定；高于阈值+0.08 的命中渐进补声纹参考 |
