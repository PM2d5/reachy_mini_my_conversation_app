...
>
> 最后核对：2026-09-26 · 新增内置人格 `default_english`：`default` 的英文专属版，机器人一律用英文回复（ask_assistant 固定话术同步英文化），其余人设与工具集不变，人格列表中钉在 `default` 之后排第二（此前：2026-09-23 · `feat/voice-speaker-identity`（三轮：长句「识别两次」收敛——早判（2 s 前缀）命中后不再做完整段复核，复核仅在早判未命中时兜底，一句话只产生一次有效判定与一条 match 日志（同段早判/复核竞态以等待早判任务+判定记忆解决）；此前 真机日志二轮修复：只有会话第一句有声纹日志——后续短句（约 1.3–1.7 s 含语义 VAD 尾静音）全部低于 1.5 s 判定下限被静默跳过；①服务端 VAD 段先做头尾静音裁剪再 embed（尾静音稀释 embedding：真人同人分 0.614 → 裁剪后应回到 0.7+，噪声房间地板 5% 峰值 RMS 或绝对 ~130 计数）；②判定下限改为裁剪后有效语音 ≥1 s（原 1.5 s 未裁剪），注册下限同步；③跳过也打 `Speaker match: skipped` 日志消盲区；④每句长话两条 match 日志（2 s 早判 + 完整段复核）不变。真机基线：同人 0.614 vs 他人 0.135（间隔 0.48，分离度极好）；此前：实测修复：两人均已注册声纹但会话中换人后归属全部落空、身份停在唤醒时人脸认出的第一人——注册后声纹参考数从不增长证实真实远场麦克风相似度达不到干净语音校准的 0.60 阈值；判定规则改为闭集比较：每人取最高余弦、赢家需 ≥ 新默认下限 0.50 且领先第二名 ≥0.08（模糊维持现任，抗摇摆），≥2.5 s 的说完话整段用完整音频复核（早判 2 s 前缀较噪可在此纠正），每次判定输出 info 级 `Speaker match:` 各人分数日志、模型加载失败由静默改为 warning；此前：修复：功能上线前已注册人脸的用户永远补不上声纹——再说「我叫X，记住我」命中 duplicate_name 直接报错、声纹注册不会执行；同名再注册改为先抓拍人脸与已有记录比对、确系本人才补挂声纹（防冒名污染），配回归测试；此前 2026-09-22：新增声纹说话人识别与按人记忆：①声纹按轮归属（§2）——本地 kaldi fbank（numpy 复刻）+ 内置 CAM++ 中文说话人 ONNX（20 万说话人、192 维、onnxruntime 单线程、零新增依赖），每个 VAD 语音段攒够 2 s 中途先判、<1.5 s 不判，换人即切会话身份+注入切换备注+广播 identity，高置信渐进补声纹（每人 5 条）；「我叫X，记住我」注册人脸同时把该句语音存为同一人声纹参考（faces.v1.json 新增 voiceEmbeddings 字段、旧数据兼容读空）；②长期记忆按人拆分（§10）——事实带可选 ownerId（face id），remember 自动挂当前身份、提示词注入按身份过滤（本人+共享池）、访客仍见全部、forget 先删可见集合再全库兜底；两项新配置 env（SPEAKER_ID_ENABLED / SPEAKER_MATCH_THRESHOLD）；此前：修复语义断句回归：semantic_vad 下 DashScope 图片注入必失败——服务端持有待判定音频时拒绝切换断句模式（`Cannot change turn_detection while input audio is pending`），其后的手动 commit 又因噪声非语音被拒（`no accepted speech`），图片进不了对话；修复为注入瞬间停送麦克风音频、先 `input_audio_buffer.clear` 再走手动断句流程；此前：新增 ask_assistant 触发场景：询问用户个人电脑相关信息（文件、磁盘、系统状态、软件配置等）时一律委托家庭助手、不得凭训练知识作答，工具描述与 default 人设规则同步新增；只读查询不受危险词拦截影响，系统级破坏性操作（删除文件/卸载等）仍在工具层直接拒答；此前：DashScope 实时模型断句升级为各家族语义级默认：Qwen3.5/3.8-omni 系 `semantic_vad`（阈值 0.5、静音 800 ms）、qwen-audio 系 `smart_turn`（声学+语义融合判轮、无额外参数，该家族仅允许在会话首段音频前设置断句模式）；旧 omni 系（qwen3-omni、qwen-omni-turbo）不支持语义 VAD、维持 `server_vad`（硬发会被拒、整条 session.update 连带工具被丢）；语义级模式下附和声（「嗯」「对对」、笑声）与噪声不再打断模型播报、只有真正插话才触发 barge-in，本地清播放队列逻辑不变（`speech_started` 只在语义确认时到达）；新配置 `DASHSCOPE_TURN_DETECTION=server_vad` 可退回纯能量 VAD、家族不支持的显式模式值记 warning 并按家族默认；HF 后端不受影响；此前：点头/摇头手势与 yes/no 表情解耦：表情规则与 `play_emotion` 参数描述中的映射由 点头→yes、摇头→no 改为 同意→yes、不同意→no，点头/摇头不再触发表情、留给模型自行组合 `move_head` 完成头部手势；此前：2026-09-19 唤醒词模型四句式混训重训：hi 与 reachy 之间的自然停顿（约 100–350ms）不再失效、单独说 "reachy" 也可唤醒（单词句式单次命中率约六成、四种句式中最弱，误报 0.71 次/小时低于旧版 1.06）；此前：人脸识别两处收敛：①匹配阈值默认 0.45→0.40——日志实测同一注册人唤醒第一帧相似度落在 0.40–0.43、稳定帧 0.9+，0.45 下时认时不认（隔三分钟一次「凯蕾，又来啦？」一次「什么？」），识别失败的会话系统提示词无身份块、问「我是谁」一无所知；②camera 拍照搭车识别在会话未识别（唤醒识别失败/超时）时认领面前人为会话用户：face 字段带 name+relation user（该会话没有身份块，模型正缺这个名字），后续拍照按已识别会话处理；视觉规则触发词补「我是谁/你认识我吗/你看到了谁」，身份类问题先走 camera 拿到名字再回答。此前：收敛对话中提用户名字的频率：身份块指令从正向「要称呼名字」改为锚定「正常人聊天很少反复称呼朋友」、明确绝大多数回复不含名字、只在打招呼/唤起注意/情绪浓的时刻用（正向指令即使带「别每句都叫」限定，实测仍导致模型几乎每轮都提名字）；camera 拍照搭车识别的 face 字段在面前是当前会话用户时不再重复带 name（系统提示词已写明用户是谁，逐次重发只会强化模型跟着说），他人/陌生人照旧带 name。再此前：新增人脸识别与身份感知：会话建立时本地识别面前的人（YuNet 检测与 MobileFaceNet ONNX embedding 均随包分发、离线秒载），识别与抬头/建连并行、最多等 3 s、失败静默按访客；命中后系统提示词插入身份块、唤醒应答带名字、广播 conversation.identity；高置信命中渐进补参考向量；`remember_face` 工具 +「我叫X，记住我」本地触发注册；camera 工具同帧搭车识别（中继模式保留 face 字段）；faces.v1.json 存储 + Web「People」管理页 + faces.* JSON-RPC；两项新配置 env。更早：待机期间丢弃迟到动作与工具结果回写的连接关闭降级、会话退出重置响应完成事件（修复拜别待机后唤醒应答被 30 s 发送超时卡住）、DashScope websocket 关闭握手 2 s 超时（网关不回应 close 帧，默认 10 s 让每次拜别待机白等满超时）、按模型家族区分的 DashScope 音色目录与跨家族音色回退、静默表情轮的语音补救（模型只回 play_emotion 不说话时补发 response.create，裸表情命令除外）、表演请求/台词情绪的本地表情触发、情绪对话主动表情、本地表情触发与表情工具去重、表情规则指令、ask_assistant / OpenClaw 对接、天线等待摆动、DashScope 摄像头图像注入、可选视觉模型的摄像头画面描述中继、视觉规则指令与会话温度配置、转头后注视保持与拍照落位等待、默认开场问候多风格随机挑选（会话无记忆，单一指令导致开场白趋同）、默认告别词新增「滚吧」））
...
- **会话身份识别**（`face_recognition.py`）：每次会话建立时（冷启动、唤醒恢复、切人格/音色重建）对面前的人做本地人脸识别——SDK YuNet 检测（`reachy_mini.vision.face_detector`；模型与 embedding 一样**随包内置**于 `vision_models/`，首次加载自动种入 HF 缓存并以离线模式瞬时读取——SDK 默认的 hub 下载在不通 HF 的网络下会重试挂死一分钟以上）取最大脸 → 双眼+鼻子 3 点仿射对齐 → 内置 MobileFaceNet ONNX embedding（onnxruntime 本地推理，与唤醒词同款机制）→ 与 `faces.v1.json` 中注册人脸做余弦匹配（阈值 `REACHY_MINI_FACE_MATCH_THRESHOLD`）。识别任务在唤醒/启动瞬间即启动，与抬头动画、websocket 建连并行；组装系统提示词前最多等 3 s，超时/无脸/模型不可用一律静默按未识别处理，绝不阻塞对话（模型加载另有 75 s 硬上限，加载线程为 daemon，断网环境也拖不死关机流程）。命中后：系统提示词在记忆块后插入「当前对话用户是X（人脸识别确认）」身份块（未识别不插入，对访客行为与从前一致，不主动盘问；指令锚定「正常人聊天很少反复称呼朋友」、绝大多数回复不含名字，只在打招呼/唤起注意/情绪浓的时刻用——正向「要称呼名字」指令即使限定「别每句都叫」，实测仍让模型几乎每轮都提名字）；唤醒应答改用带名字的熟人变体池（`prompts.py` 的 `KNOWN_USER_WAKE_ACKNOWLEDGEMENT_PROMPTS`，独立轮换索引）；向 JSON-RPC 客户端广播 `conversation.identity`。高置信命中（阈值+0.08）把该帧 embedding 追加进此人参考集（每人上限 5、丢最旧，外观变化自动跟上）并刷新 lastSeen。注册：用户说「我叫X，记住我」（中英句式）→ 模型调 `remember_face` 工具，或本地触发兜底（`match_face_enrollment_command`，与表情指令同机制，裸「记住我」不触发、留给模型追问名字后正常调工具；60 s 同名去重防双发）；等头部动作落位后连拍 3 帧、≥2 帧检出人脸才入库，同名拒绝、最多 10 人。`camera` 工具拍照时同帧顺带识别并更新 lastSeen，结果附 `face: {name, relation: user|other|unknown}` 字段（会话未识别时命中即认领该人为会话用户、face 带 name——该会话系统提示词没有身份块，模型正缺这个名字；已识别会话的 relation 为 user 时不再带 name——系统提示词已写明用户是谁，逐次重发名字只会强化模型反复说；b64 图片字段照旧剥离，face 字段模型可见；DashScope 视觉中继模式下并入 caption 结果不丢失），让模型自然感知「面前换人了」。
- **声纹按轮归属**（`audio/speaker_id.py`，两个后端共享该循环）：人脸识别只回答「面前是谁」（同框多人时永远取最大脸），声纹回答「**谁在说话**」——解决两人同框轮流对话被当成同一个人的问题。前端为 numpy 复刻的 kaldi fbank（80 mel、25/10 ms、povey 窗、snip_edges=false，与 sherpa-onnx 对 3D-Speaker 模型的配置逐参一致）+ 内置 CAM++ 中文说话人 ONNX（`audio/models/speaker_embedding_campplus_zh.onnx`，20 万中文说话人训练、onnxruntime 单线程本地推理、192 维 embedding，与唤醒词/人脸模型同款离线机制），无新增 Python 依赖。**触发时机**：每个用户语音段（服务端 VAD `speech_started/stopped` 之间）缓冲麦克风音频，攒够 2 s 即在说话中途先判一次（赶在服务端生成本轮回复之前，换人后第一声称呼就是对的）；说话结束时不足 2 s 但 ≥1.5 s 的段在 `speech_stopped` 补判；不足 1 s 有效语音的应答（「嗯」「对」）不判定、维持上一身份；段尾静音先裁掉再 embed，裁剪后过短的段记 info 级 `Speaker match: skipped` 日志——真机实测：语义 VAD 尾静音把同人相似度从 0.75+ 稀释到 0.614，且多数自然短句（如「你知道我是谁吗？」约 1.3 s 含尾静音）落在旧 1.5 s 下限之下被静默跳过；判定为**闭集比较 + 绝对下限**：对每个注册人取最高余弦，赢家需同时满足 ≥ 绝对下限（`REACHY_MINI_SPEAKER_MATCH_THRESHOLD`，默认 0.50——远场真实麦克风把同人分数拉低于干净语音校准值，实测 0.60 下两人都从未命中、每轮归属落空）**且**领先第二名 ≥0.08（两人分数接近视为模糊、维持现任身份，天然抗摇摆）；每次判定以 info 级日志输出各人分数（`Speaker match: …`），供现场调阈值。说完话后 ≥2.5 s 的整段仅在**早判未命中**时用完整音频复核一次（2 s 早判窗口音频少、更噪；早判已命中则不复核——一句话只产生一次有效判定，不会出现同一句识别两次）。**换人动作**：更新会话身份（`source="voice"`）、广播 `conversation.identity`（Web 徽章跟切）、并按 camera 搭车识别同款方式注入一条上下文条目（`SPEAKER_SWITCH_NOTE`，含"别反复提名字"锚定）——系统提示词的身份块仍写着会话开始时那个人，切换需要自己的信号；认出的人=当前身份时零动作，仅在后台渐进补声纹参考（阈值+0.08、每人上限 5，与人脸同机制；站姿/坐姿、不同距离的声音慢慢进参考库）。推理每轮一次、后台线程几十毫秒、不碰音频循环。**注册**：「我叫X，记住我」在注册人脸的同时把该句语音（≥1 s 即可作种子，短于 1 s 跳过并记日志）embed 后存为同一人的声纹参考（`faces.v1.json` 条目新增 `voiceEmbeddings` 字段，无人脸时无存储、旧数据无此字段读为空、Web People 页照旧）；**此功能上线前已注册人脸的人再说同一句会命中同名拒绝——修复为同名再注册时先抓拍当前人脸与已有记录比对，确系本人才把该句声纹补挂到已有的人身上（人脸验证挡住冒名污染声纹；不是本人则照旧报“名字已注册”）**；`REACHY_MINI_SPEAKER_ID_ENABLED=0` 整体关闭。
- **聆听与打断**：服务端 VAD 负责断句；用户一开口（`speech_started`）即本地清空播放队列实现真打断（barge-in），同时冻结天线动作表示"在听"。DashScope 后端按模型家族默认语义级断句：Qwen3.5/3.8-omni 系 `semantic_vad`（阈值 0.5、静音 800 ms，阿里官方推荐）、qwen-audio 系 `smart_turn`（声学+语义融合判轮、无额外参数；该家族仅允许在会话首段音频前设置断句模式）；旧 omni 系（qwen3-omni、qwen-omni-turbo）不支持语义 VAD、维持 `server_vad`。语义级模式下附和声（「嗯」「对对」、笑声）与背景噪声不再误触发打断，`speech_started` 只在语义确认的真插话时到达、本地清队逻辑不变；`DASHSCOPE_TURN_DETECTION=server_vad` 可整体退回（家族不支持的显式模式值记 warning 并按家族默认）。HF 后端固定 `server_vad` 不受影响。
...
- **工具调用**：模型发起的函数调用全部后台执行（见 §7）；结果回填为 `function_call_output`，仅在工具出错、`needs_response=True`、或**静默表情轮补救**（模型把整轮只回成一个 `play_emotion` 调用而本轮无任何语音——后端收到工具结果不会自动续话，如「讲一个开心的故事」被只回了个表情就整轮哑掉；此时补发一次 `response.create` 让模型把话说完，但用户要的是裸表情命令「做个 X 的表情」时保持静默，本轮已有音频也不重复应答）时触发一次语音跟进；`camera` 拍到的 JPEG 会作为图片消息重新注入多模态对话。所有人格的会话指令以「视觉规则」开头（`prompts.py` 的 `CAMERA_TOOL_RULE`，含中文触发词与少样本示例）：camera 工具就是模型的眼睛，视觉与身份类请求（含「我是谁/你认识我吗/你看到了谁」，先拍照拿到识别名字再回答）必须先调工具、只依据照片作答，禁止凭空描述或谎称没有摄像头——多模态实时模型（如 Qwen-Omni）否则会直接幻觉作答或拒答而不调工具；该规则置于指令最前（实测置顶 5/5、置尾 3/5 命中工具调用）；规则同时要求：用户问某个方向（左/右/上/下）有什么时，先 `move_head` 转向、再 `camera` 拍摄，禁止拿正面照片冒充某个方向。视觉规则之后紧跟「表情规则」（`prompts.py` 的 `EMOTION_TOOL_RULE`，同款结构与少样本示例）：用户让机器人展示情绪/表情时，必须同轮先调 `play_emotion`（含中文情绪词→意图映射，同意→yes、不同意→no，点头/摇头不在映射内、由模型自行组合 `move_head`，无匹配则 `random`），再用该情绪语气简短回应，禁止只用声线"扮演"情绪——实测日志中 `play_emotion` 从未被模型调用过（camera/move_head/dance 均正常命中），用户看到的就是"没做表情"。规则同时覆盖**主动表情**：模型自己的回复带明确强烈情绪时（安慰、道贺、道歉、共情），须在说话同轮调用 `play_emotion`；中性/事务性轮次明确跳过（防每轮都演、显得机械）。表情规则之外还有一层**本地表情触发**（`huggingface_realtime.py` 的 `_maybe_play_expression_command`，两个后端共享该循环）：用户转写一完成就用 `match_expression_command()`（`tools/play_emotion.py`）做本地匹配，只认明确指令句式（中文"做/来/弄/表演…表情/情绪/动作"、英文 "do/show/make a … emotion/face/expression"）**以及点名情绪的表演请求**（"讲一个悲伤的故事""唱首开心的歌""讲个笑话→happy"；无情绪词的"给我讲个故事"不触发，留给模型层），命中即以 idle 工具调用方式直接入队 `play_emotion`（结果不回填模型对话，模型照常语音回应）；情绪词→意图映射同时写进了工具参数描述。动机：实测 DashScope 全部模型与 HF 端点的中文会话都不会自发调用该工具（HF+英文才会），本地触发保证明确指令场景在后端×语言全组合下 100% 生效。**台词情绪监听**（`_watch_spoken_emotion`）：助手语音转写增量实时流经共享会话循环，用同一词表扫描模型自己说出的台词，命中即在说话的同时入队对应表情（每轮最多一次，`response.output_audio_transcript.done` 全文兜底；否定式"不开心"跳过；单字词因易误命中"困难/拖累"不参与；"别难过"这类安慰句按命中处理，机器人以关切表情回应）。词法匹配天然只认说出口的情绪词——全程无情绪词的含蓄叙事不触发，那是模型层（表情规则）的职责。配套去重：`PlayEmotion` 4 秒窗口内同名动作只入队一次（本地触发与模型调用谁先到都只播一遍）。
...
| 断句/打断 | `server_vad`（能量 VAD） | 按家族默认语义级断句：Qwen3.5/3.8-omni 系 `semantic_vad`（阈值 0.5、静音 800 ms）、qwen-audio 系 `smart_turn`（无额外参数；仅可在会话首段音频前设置模式）——附和声与噪声不打断播报、只有真插话触发 barge-in；旧 omni 系与 `DASHSCOPE_TURN_DETECTION=server_vad` 维持/回退 `server_vad` |
| 视觉 | `camera` 拍照以 `input_image` 消息注入对话 | 连接层把 `input_image` 消息翻译为 `input_image_buffer.append`（纯 base64，单帧/单图 ≤256 KB；超限自动降到 720p/更低质量重编码，仍超限则丢弃并记 error）+ `input_audio_buffer.commit` 提交进对话；因断句 VAD 只随语音提交图片、且语义断句下输入缓冲有待判定音频时拒绝切换断句模式，注入瞬间先短暂停送麦克风音频并清空输入缓冲，再切手动断句并附 1 s 合成噪声（均仅上送服务器，不外放），随后恢复原断句配置。**可选视觉描述中继**：设置了 `DASHSCOPE_VISION_MODEL` 时不注入图片，改由该视觉模型（`vision_relay.py`）对画面+问题做图像描述，描述文字作为 `function_call_output` 回填——供看不见图片的 realtime 模型（qwen-audio 系）使用；描述失败按错误回填，不会中断对话 |
...
- **standby**：实时会话暂停，机器人缩头下沉（头部保持水平、不低头，与真正的休眠姿态区分；天线垂落静止，2 s 插值），期间抑制呼吸动作，并丢弃待机期间新排队的动作（如告别后模型迟到的 `play_emotion` 不会再把头顶起来）；麦克风只喂给离线唤醒词检测器（openWakeWord，ONNX，16 kHz）。默认唤醒词 **"hi reachy"**（两词间的自然停顿可容忍），由内置自训模型 `audio/models/hi_reachy.onnx` 检测；该模型用流利、带停顿（逗号/省略号变体）与单词四种句式混训，因此**单独说 "reachy" 也能唤醒**（单词形式单次命中率约六成，连说两三次即稳定命中，是最弱句式）；说出唤醒词即抬头回中立位（1.5 s 插值）并恢复会话，以极简应答代替重新问候（`WAKE_ACKNOWLEDGEMENT_PROMPTS`：两三个词、跟所讲语言一致，如"我在""干嘛"；每次唤醒是无记忆的新会话，由应用在几种应答风格间轮换并随机起步，保证说法有变化）。唤醒瞬间并行启动人脸识别（与抬头/建连同时进行，见第 2 节），识别到注册用户时唤醒应答改为带名字的熟人变体。
...
| `remember` | `fact`（必填，一句以内） | 保存用户事实到长期记忆（§10），自动挂当前会话身份（人脸/声纹识别出的人）；注册"人本身"（人脸+声纹）改用 `remember_face` |
| `forget` | `query`（必填，子串匹配） | 删除一条记忆（先删当前用户可见集合、找不到再全库兜底），多条时报告其他候选 |
| `remember_face` | `name`（必填，≤24 字符） | 注册说话人的脸与声纹供日后识别打招呼（见 §2 会话身份识别/声纹按轮归属）：等头部落位后连拍 3 帧、≥2 帧检出人脸才入库，入库后同句语音（≥1 s）存为声纹参考；同名拒绝、满 10 人拒绝（已有同名者再注册：人脸验证通过则补挂声纹）；「我叫X，记住我」另有本地触发兜底，60 s 同名去重；`REACHY_MINI_FACE_RECOGNITION_ENABLED=0` 时对模型隐藏；删除/重命名走 Web「People」页（§11） |
| `idle_do_nothing` | `reason`（可选） | 空闲轮次保持静止；结果不回传模型 |
| `go_to_sleep` | 无 | 机器人入睡并停止应用（仅在用户明确要求时） |
| `ask_assistant` | `query`（必填） | 把复杂任务委托给家庭助手 OpenClaw（实时信息查询、日程提醒、长期家庭记忆、多步规划、用户个人电脑信息查询）。最近对话（含上次结果）由系统自动注入（5 轮、每轮 200 字），模型只需传 query——history 不进工具 schema，避免 Qwen 截断长函数参数。回复经 markdown/emoji 清洗后回传模型转述；调用期间进入静默等待态（见 §2）。危险指令（删除文件/卸载/发消息/花钱）在工具层直接拦截拒答，不发请求；超时或网络错误返回 `{"ok": false, "error": ...}`，由模型播报兜底话术。需配置 `OPENCLAW_API_URL` + `OPENCLAW_API_TOKEN`，未配置时该工具对模型隐藏 |
...
- **来源**：`profiles/` 内置 14 个可见人格（default、default_english（default 的英文专属版，一律用英文回复）、bored_teenager、captain_circuit、chess_coach、cosmic_kitchen、hype_bot、mad_scientist_assistant、mars_rover、nature_documentarian、noir_detective、sorry_bro、time_traveler、victorian_butler；`tedai` 为 hidden）+ 外部目录 + 用户自建（`user_personalities/` 前缀，Web 端可增删改）。
...
- **列表顺序**：`default` 固定首位，`default_english` 钉在其后，其余按目录名字母序；用户自建人格追加在末尾。
...
- **按人拆分**：每条事实带可选 `ownerId`（注册人的 face id）。`remember` 保存时自动挂当前会话身份（人脸或声纹识别出的人）；未识别会话存入无主共享池。提示词注入按身份过滤——识别到的人只看自己的事实+共享池，未识别访客仍看全部（与从前行为一致）；旧数据无 `ownerId` 字段一律视为共享。
- 由 `remember` / `forget` 工具读写；每条带 id、创建时间；forget 按大小写不敏感子串匹配（识别用户先在其可见集合内找、找不到再全库兜底，误归属的事实不会变成删不掉）。
- 每次会话（重建）时可见事实以条目列表注入系统提示词开头，指示模型自然利用而非背诵。
...
**服务端通知**（服务器 → 客户端推送）：`conversation.transcript {role, text, final}`、`conversation.turn {state}`、`conversation.phase {phase, reason}`、`conversation.activity {reason}`、`conversation.identity {name, source}`（会话建立时人脸识别结果，`name` 为空即访客；会话中声纹换人也即时重发，`source` 为 face 或 voice）、`conversation.level {role, rms}`（约 15 Hz 音量表，驱动光球）。
...
| `DASHSCOPE_TURN_DETECTION` | Qwen3.5/3.8-omni 系 `semantic_vad`，qwen-audio 系 `smart_turn`，旧 omni 系 `server_vad` | DashScope 断句模式：`semantic_vad`（Qwen3.5/3.8-omni 语义断句）、`smart_turn`（qwen-audio 系声学+语义融合判轮，无额外参数）或 `server_vad`（纯能量 VAD，旧行为）——语义级模式下附和声「嗯」「对对」、笑声与噪声不再打断模型播报，只有真正插话触发 barge-in；家族不支持的显式值记 warning 并按家族默认；qwen-audio 系仅允许在会话首段音频前设置断句模式 |
...
| `REACHY_MINI_FACE_MATCH_THRESHOLD` | `0.40` | 人脸余弦匹配阈值（0–1，调高更严格）；高于阈值+0.08 的命中会渐进补参考向量 |
| `REACHY_MINI_SPEAKER_ID_ENABLED` | `1` | 声纹按轮归属总开关；0 关闭每轮说话人识别与声纹注册（见 §2 声纹按轮归属） |
| `REACHY_MINI_SPEAKER_MATCH_THRESHOLD` | `0.50` | 声纹余弦匹配下限（0–1，调高更严格）；赢家还需领先第二名 ≥0.08 否则维持现任身份；先裁掉段头/段尾静音（服务端 VAD 段尾部最多带 ~0.8 s 静音，实测会把同人分数从 0.75+ 稀释到 0.614）、有效语音不足 1 s 不判定（有 `skipped` 日志）；高于阈值+0.08 的命中渐进补声纹参考；调优看 `Speaker match:` 日志 |
