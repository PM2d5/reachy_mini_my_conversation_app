...
| `remember_face` | `name`（必填，≤24 字符） | 注册说话人的脸与声纹供日后识别打招呼（见 §2 会话身份识别/声纹按轮归属）：等头部落位后连拍 3 帧、≥2 帧检出人脸才入库，入库后同句语音（≥1 s）存为声纹参考；同名拒绝、满 10 人拒绝；「我叫X，记住我」另有本地触发兜底，60 s 同名去重；`REACHY_MINI_FACE_RECOGNITION_ENABLED=0` 时对模型隐藏；删除/重命名走 Web「People」页（§11） |
